        ...
        buttons: {
            ...
            "отправить в CRM": {
                ...
                onClick: function() {
                    ...
                    /*var $comment_send_button = $($this).find(".comment_send_button");
                    $($comment_send_button).click();*/
                    var $active_block = $this;
                    var $text_comment = $($this).find(".comment_send textarea").val();

                    $.fn.send_comment = function() {
                        $.ajax({
                            url : "/controller",
                            type : "POST",
                            data : {
                            ...
                            success: function(data) {
                                ...
                                if ($($comments).length == 0) {
                                    ...
                                    $.each($($active_block).find(".trouble_comments"), function() {
                    ...
                    $(this).send_comment().ajaxStop($.fn.send_trouble());
                }
            }
        }
    });
    ...
    function check_timeout() {
        $.each($("#admin_trouble_list div[id$='_timeout']"), function() {
            if ($.trim($(this).text()) != "") {
                ...
                var curr_date = new Date();
                if (date < curr_date) {
                    $(this).parent().parent().find("h3").css({background: "#000", color: "#FFF", border: "3px solid red"});
                    $(this).parent().parent().find("h3 .title_timeout").text("Срок устранения аварии истёк");
                    $(this).parent().parent().find("h3 .title_timeout").css({color: "red", "text-align": "right"});
                }
            }
        });
    }
